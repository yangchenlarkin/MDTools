...
@implementationProtocol(ModuleManager)
...
- (BOOL)_isSuperModuleManagerOfViewController:(UIViewController *)viewController {
    id <ModuleManager> mm = viewController.moduleManager;
    while (mm) {
        if (mm == self) {
            return YES;
        }
                           ...
                           block:^(id<NSObjectAspectInfo> info) {
                               ...
                               dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                                   if (self.tailViewController == lastViewController) {
                                       ...
                                       [RACObserve(self._lastViewControllerContainer, object) subscribeNext:^(id  _Nullable x) {
                                           typeof(weakSelf) self = weakSelf;
...
- (UINavigationController *)navigationController {
    id (^block)() = objc_getAssociatedObject(self, "__navigationController");
...
- (void)preprocessWithFinish:(void (^)(id error))finish {
    finish(nil);
}
...
- (id<ModuleManager>)superModuleManager {
    return objc_getAssociatedObject(self, "__superModuleManager");
}

- (void)setSuperModuleManager:(id<ModuleManager>)superModuleManager {
